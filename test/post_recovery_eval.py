...
def upload_asset(repo, token, local_path, remote_path):
    """Create or update a file on the ci-assets branch; return its raw URL (or None)."""
    ...
    payload = {'message': f'update {remote_path}', 'content': content, 'branch': ASSETS_BRANCH}
...
def main():
    ...
    for arg in sys.argv[1:]:
        ...
        entries.append((label, out_dir, aggregate(runs)))
    if not entries:
        print('no scenarios given', file=sys.stderr)
        return 2

    rows = '\n'.join(row(lbl, a) for lbl, _, a in entries)

    # Upload each scenario's convergence-curve plot to the ci-assets branch and
    # embed it inline (skipped silently if a plot is missing or upload fails).
    plots = ''
    for i, (label, out_dir, _) in enumerate(entries):
        url = upload_asset(
            repo, token, os.path.join(out_dir, 'convergence_plot.png'),
            f'pr{pr}_recovery_{i}.png')
        if url:
            plots += f'\n**{label} — translation error over time:**\n\n![{label}]({url})\n'
